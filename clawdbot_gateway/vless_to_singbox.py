#!/usr/bin/env python3
"""Build sing-box JSON config from a VLESS share URI.
...
def build_vless_outbound(uri: str) -> dict[str, Any]:
    ...
    if security in {"tls", "reality"}:
        ...
        server_name = _last(query, "sni") or _last(query, "host")
        if server_name:
        ...
        if security == "reality" and not fingerprint:
            # sing-box requires uTLS for reality; chrome is the de-facto safe default.
            fingerprint = "chrome"
        if fingerprint:
    ...
    transport = (_last(query, "type") or "tcp").strip().lower()
    if transport and transport not in {"tcp", "none"}:
        ...
        if transport in {"ws", "websocket"}:
            ...
            path = _last(query, "path")
            if host:
                transport_config["host"] = _split_csv(host)
            if path:
                transport_config["path"] = unquote(path)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--listen-port", type=int, default=7890, help="Local mixed proxy port")
